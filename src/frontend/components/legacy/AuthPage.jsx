    ...
    >
      ...
      <div className="flex justify-end mt-3">
        <button
          ...
          style={{ color: C.sub, fontFamily: FB }}
        >
          Forgot password?
        </button>
      </div>
      ...
      {authError && (
        <p className="text-xs mt-3 text-center" style={{ color: C.brick, fontFamily: FB }}>
          {authError}
        </p>
      )}
      ...
      <button
        type="submit"
        disabled={submitting}
        className="w-full mt-6 py-3.5 rounded-full text-white text-sm font-semibold flex items-center justify-center gap-2 transition-all duration-200 hover:scale-[1.015] active:scale-95 disabled:opacity-60"
        style={{ background: C.brick, fontFamily: FB }}
      >
        {submitting ? <Loader2 size={15} className="animate-spin" /> : <>Log in <ArrowRight size={15} /></>}
      </button>
  ...
  return (
      ...
      >
        <div className="absolute top-8 left-8 z-30 flex flex-col items-start">
          ...
          <Link href="/" className="relative group mt-0.5 ml-2 cursor-pointer">
